...
import { CalendarDays, GraduationCap, Video, FileText } from "lucide-react";
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {enrollments.map((e: any) => {
            const isSelfPaced = e.course?.course_format === "self_paced";
            return (
              <div key={e.id} className="flex items-center justify-between rounded-lg border border-sumi-100 p-4">
                <div>
                  ...
                  <p className="text-xs text-sumi-400">
                    {isSelfPaced ? "Khóa tự học" : e.course?.schedule_text} · GV {e.course?.teacher?.full_name}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <Badge tone={STATUS_TONE[e.status]}>{STATUS_LABEL[e.status]}</Badge>
                  {isSelfPaced && e.status === "approved" && (
                    <Link href={`/my-courses/${e.course_id}`}>
                      <Button size="sm">
                        <GraduationCap className="h-4 w-4" /> Vào học
                      </Button>
                    </Link>
                  )}
                </div>
