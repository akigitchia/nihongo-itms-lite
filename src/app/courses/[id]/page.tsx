...
import { CalendarDays, Users, GraduationCap, Wallet } from "lucide-react";
...
export default async function CourseDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      ...
      {isSelfPaced ? (
        <div className="mt-6 grid grid-cols-2 gap-4 rounded-xl border border-sumi-100 p-4">
          <div>
            <div className="flex items-center gap-1.5 text-xs text-sumi-400">
              <Wallet className="h-3.5 w-3.5" /> Học phí
            </div>
            <p className="mt-1 text-sm font-semibold text-sumi-900">{formatVnd(course.price)} (trả 1 lần)</p>
          </div>
          <div>
            <div className="flex items-center gap-1.5 text-xs text-sumi-400">
        ...
        {user && myEnrollment && (
          <Alert tone={myEnrollment.status === "approved" ? "success" : myEnrollment.status === "rejected" ? "error" : "info"}>
            ...
            {myEnrollment.status === "approved" && (
              <>
                {" "}
                — <Link href="/dashboard" className="underline">Xem khóa học của bạn</Link>
              </>
            )}
          </Alert>
        )}
        {user && !myEnrollment && (isSelfPaced || seatsLeft > 0) && (
          <>
            {isSelfPaced && course.payment_note && (
              <Alert tone="info" className="mb-3">
                <strong>Hướng dẫn thanh toán:</strong> {course.payment_note}
              </Alert>
            )}
            <RegisterButton courseId={course.id} />
          </>
        )}
        {user && !myEnrollment && !isSelfPaced && seatsLeft <= 0 && <Alert tone="info">Lớp đã đủ số lượng học viên tối đa.</Alert>}
      </div>
    </div>
  );
}
